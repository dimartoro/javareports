...
public class DAOProperties {
    ...
    private static final String PROPERTIES_FILE = "com/sms/sap/report/employee/config/dao.properties";
    ...
    static {
        try {
	        CodeSource codeSource = DAOProperties.class.getProtectionDomain().getCodeSource();
	        File jarFile = new File(codeSource.getLocation().toURI().getPath());
	        File jarDir = jarFile.getParentFile();
	        if (jarDir != null && jarDir.isDirectory()) {
                File propFile = new File(jarDir, "dao.properties");
                PROPERTIES.load(new BufferedReader(new FileReader(propFile.getAbsoluteFile())));
            }
